...
当前项目增加了最小 extraction quality control：如果图片表格抽取结果置信度低，会生成 content_type=image_table_review_required 的人工复核 Document。
这不是 RAG 检索阶段的 low_confidence，而是入库阶段的质量控制。
判断信号包括 OCR confidence、表头完整性、行列一致性、空单元格比例、关键字段缺失。
