...
修改资料、chunk 逻辑、metadata 结构或版本号后，执行：
...
## 3. Document Ingestion 专项测试（I001-I010）
...
| I002 | Processor | `scripts/test_document_processor.py` | 清洗后 Document 数量不变 | 空格 / 空行处理 | 是 | 通用清洗 |
...
| I005 | PDF Loader | `scripts/test_pdf_loader.py` | PDF 3 页生成 3 个 Document | `page=1/2/3` | 是 | 文本型 PDF |
| I006 | PDF 条款切分测试 | `scripts/test_pdf_clause_split.py` | 第 2 页切出 A/B/C/D | 条款数量 | 是 | 测试 policy_clause |
| I007 | 目录 Loader | `scripts/test_document_dir_loader.py` | txt + pdf 合并为 list[Document] | Document 总数 | 是 | 批量扫描 |
...
4. 报销500到2000元怎么审批？ → 那再高一点呢？
   → 展示多轮 Query Rewrite

5. 公司年终奖发放规则是什么？
   → 展示 low_confidence 防胡编

6. LLM_PROVIDER=ollama / deepseek 分别请求同一个 RAG 问题
   → 展示最终回答模型来源切换
```
...
9. 资料目录扫描可以同时读取 txt + pdf
10. policy_clause 可以对通用“xxx制度条款A/B/C”结构做条款级切分
11. used_chunks_debug 可以追溯 source_file / file_type / page / chunk_strategy
12. DeepSeek / Ollama 最终回答模型切换正常
```
